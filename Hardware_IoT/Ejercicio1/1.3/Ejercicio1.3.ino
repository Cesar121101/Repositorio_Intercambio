//Incluir definiciones de sensores, leds y botones
#include "hardware.h"
...
int medicionSensor(int pin){
  int promedio = 0;
  for(int i = 0; i<10; i++){
    promedio += analogRead(pin);
  }
  return promedio/10;
}

void loop() {
  //Medicion del acelerometro
  sensor_x = medicionSensor(x);
  sensor_y = medicionSensor(y);
  sensor_z = medicionSensor(z);
  ...
  if (tiempo_actual - tiempo_anterior >= 400){
    ...
    if(sensor_x > 525 and sensor_x < 585 and sensor_y > 420 and sensor_y < 465){ 
