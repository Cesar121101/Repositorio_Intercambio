...
void THPWM_test (void *argument);                   // thread function

int init_PWM_test(void) {
	osThreadId_t PWM_test = osThreadNew(THPWM_test, NULL, NULL);
  if (PWM_test == NULL) {
    return(-1);
  }
	queue_pwm = osMessageQueueNew(8, 16, NULL);
  return (0);
}


void THPWM_test(void *argument) {
  msgQueue_PWM localObject;
	localObject.Tr = 25.0;
	localObject.Tm = 18.0;
	while (1) {
		osDelay(1000);
		localObject.Tm += 0.3;
		if(localObject.Tm > 35.0){
			localObject.Tm = 17.0;
		}
		osMessageQueuePut(queue_pwm, &localObject, NULL, osWaitForever);
